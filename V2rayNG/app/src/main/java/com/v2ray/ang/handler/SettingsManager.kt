...
object SettingsManager {
    ...
    @Volatile
    private var hevTunLibraryAvailable: Boolean? = null
    ...
    fun isUsingHevTun(): Boolean {
        val enabled = MmkvManager.decodeSettingsBool(AppConfig.PREF_USE_HEV_TUNNEL, false)
        if (!enabled) {
            return false
        }
        val available = isHevTunLibraryAvailable()
        if (!available) {
    ...
    private fun isHevTunLibraryAvailable(): Boolean {
        hevTunLibraryAvailable?.let { return it }
        return synchronized(this) {
            hevTunLibraryAvailable?.let { return@synchronized it }
            runCatching {
                System.loadLibrary("hev-socks5-tunnel")
                true
            }.getOrElse {
                LogUtil.w(AppConfig.TAG, "hev-socks5-tunnel library missing; falling back to Xray TUN", it)
                false
            }.also { hevTunLibraryAvailable = it }
        }
    }
