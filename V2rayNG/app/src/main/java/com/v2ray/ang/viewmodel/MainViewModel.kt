...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun testHbrayRealPing(successLimit: Int) {
        MessageUtil.sendMsg2TestService(
            getApplication(),
            TestServiceMessage(key = AppConfig.MSG_MEASURE_CONFIG_CANCEL)
        )
        val guids = MmkvManager.decodeServerList(HbrayManager.SUBSCRIPTION_ID)
        MmkvManager.clearAllTestDelayResults(guids)
        updateListAction.value = -1
        viewModelScope.launch(Dispatchers.Default) {
            if (guids.isEmpty()) {
                withContext(Dispatchers.Main) {
                    updateTestResultAction.value = getApplication<AngApplication>().getString(R.string.hbray_no_nodes)
                }
                return@launch
            }
            MessageUtil.sendMsg2TestService(
                ...
                TestServiceMessage(
                    key = AppConfig.MSG_MEASURE_CONFIG_START,
                    subscriptionId = HbrayManager.SUBSCRIPTION_ID,
                    successLimit = successLimit,
                    pruneToSuccessLimit = true,
                    validateStartupConfig = true
                )
            )
        }
    }
    ...
    fun onTestsFinished() {
        viewModelScope.launch(Dispatchers.Default) {
            ...
            withContext(Dispatchers.Main) {
                reloadServerList()
                if (subscriptionId == HbrayManager.SUBSCRIPTION_ID) {
                    val availableCount = HbrayManager.availableGuidList().size
                    updateTestResultAction.value = if (availableCount > 0) {
                        getApplication<AngApplication>().getString(R.string.hbray_ready_count, availableCount)
                    } else {
                        getApplication<AngApplication>().getString(R.string.hbray_no_usable_after_test)
                    }
                }
            }
        }
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_MEASURE_CONFIG_NOTIFY -> {
                    val content = intent.getStringExtra("content")
                    updateTestResultAction.value = if (subscriptionId == HbrayManager.SUBSCRIPTION_ID) {
                        getApplication<AngApplication>().getString(R.string.hbray_test_progress, content)
                    } else {
