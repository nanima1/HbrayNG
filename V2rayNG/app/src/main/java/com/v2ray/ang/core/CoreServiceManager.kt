...
//        if (!result.status) error(result.errorMessage.ifBlank { "Failed to get V2Ray config" })
     ...
     */
    fun startCoreLoop(vpnInterface: ParcelFileDescriptor?, notifyFailure: Boolean = true): Boolean {
        if (coreController.isRunning) {
            LogUtil.w(AppConfig.TAG, "StartCore-Manager: Core already running")
            return false
        }
        ...
        if (service == null) {
            ...
            lastStartFailureMessage = "Service is null"
            return false
        }
        ...
        try {
            ...
            cleanupAfterFailedStart(service)
            HbrayManager.markStartupFailed(MmkvManager.getSelectServer())
            if (notifyFailure) {
    ...
    private fun cleanupAfterFailedStart(service: Service) {
        try {
            coreController.stopLoop()
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "StartCore-Manager: Failed to clean up failed core loop", e)
        }
        try {
            service.unregisterReceiver(mMsgReceive)
        } catch (_: Exception) {
            // Receiver may not have been registered if config generation failed early.
        }
        CoreNativeManager.reconcileBrowserDialer("")
        try {
            browserDialer?.stop()
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "StartCore-Manager: Failed to stop browser dialer after failed start", e)
        }
        browserDialer = null
        currentConfig = null
    }
