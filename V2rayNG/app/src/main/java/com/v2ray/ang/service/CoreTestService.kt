    ...
    private fun handleMeasureStart(message: TestServiceMessage, startId: Int) {
        ...
        if (guidsList.isNotEmpty()) {
            ...
            worker = RealPingWorkerService(
                ...
                validateStartupConfig = message.validateStartupConfig,
    ...
    private fun handleWorkerEvent(event: RealPingEvent, message: TestServiceMessage, onWorkerDone: () -> Unit) {
        when (event) {
            is RealPingEvent.Progress -> {
                val progressText = if (message.subscriptionId == HbrayManager.SUBSCRIPTION_ID && message.successLimit > 0) {
                    getString(R.string.hbray_test_progress, event.text)
                } else {
                    getString(R.string.connection_runing_task_left, event.text)
                }
            ...
            is RealPingEvent.Finish -> {
                if (event.status == "0" && message.pruneToSuccessLimit && message.successLimit > 0) {
                    HbrayManager.keepTopAvailableAndSelect(this, message.successLimit)
                }
