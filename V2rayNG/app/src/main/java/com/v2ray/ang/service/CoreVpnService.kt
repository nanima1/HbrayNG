...
private const val HBRAY_START_RETRY_LIMIT = 5
...
class CoreVpnService : VpnService(), ServiceControl {
    ...
    private var hbrayStartRetries = 0
    private var lastVpnSetupFailureMessage: String? = null
...
//    }
    ...
    override fun startService() {
        if (!::mInterface.isInitialized) {
            notifyStartFailure("VPN interface not initialized")
            stopAllService()
            return
        }
        val startedFromHbray = HbrayManager.isHbrayGuid(MmkvManager.getSelectServer())
        if (!startCoreWithHbrayFallback(startedFromHbray)) {
            val message = CoreServiceManager.lastStartFailureMessage ?: "Failed to start core loop"
            LogUtil.e(AppConfig.TAG, "StartCore-VPN: $message")
            if (startedFromHbray) {
    ...
    private fun startCoreWithHbrayFallback(startedFromHbray: Boolean): Boolean {
        while (true) {
            if (CoreServiceManager.startCoreLoop(mInterface, notifyFailure = !startedFromHbray)) {
                hbrayStartRetries = 0
                return true
            }
            ...
            hbrayStartRetries += 1
            if (hbrayStartRetries >= HBRAY_START_RETRY_LIMIT) {
                return false
            }
            ...
            val nextGuid = MmkvManager.getSelectServer()
            if (nextGuid == failedGuid || !HbrayManager.isHbrayGuid(nextGuid)) {
                return false
            }
            LogUtil.w(AppConfig.TAG, "StartCore-VPN: Hbray node failed, retrying next tested node")
        }
    }
    ...
    private fun setupVpnService(): Boolean {
        lastVpnSetupFailureMessage = null
        val prepare = prepare(this)
        if (prepare != null) {
            notifyStartFailure("VPN permission is not granted")
            stopAllService()
            return false
        }

        if (configureVpnService() != true) {
            notifyStartFailure(lastVpnSetupFailureMessage ?: "VPN configuration failed")
            stopAllService()
            return false
        }
    ...
    private fun configureVpnService(): Boolean {
        ...
        try {
            ...
            lastVpnSetupFailureMessage = e.message?.takeUnless { it.isBlank() } ?: "Failed to establish VPN interface"
        }
        return false
    }
    ...
    private fun notifyStartFailure(message: String) {
        LogUtil.e(AppConfig.TAG, "StartCore-VPN: $message")
